...
int main(int argc,char *argv[] ){
    ...
    int recvbuff=atoi(argv[3]);
    int len = sizeof(recvbuff);
    setsockopt(sockfd,SOL_SOCKET,SO_SNDBUF,&recvbuff,sizeof(recvbuff));
    getsockopt(sockfd,SOL_SOCKET,SO_SNDBUF,&recvbuff,(socklen_t* )&len);
    ...
    if(connfd<0) printf("errno is %d\n",errno);
        ...
        while(recv(connfd,buffer,BUF_SIZE-1,0));
        close(connfd);
    }
